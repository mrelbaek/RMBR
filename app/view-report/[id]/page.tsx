    ...
    async function fetchReport() {
      ...
      try {
        ...
        if (result.error) {
          ...
          // Check if the report has a source marker
          if (result.report.includes("[Generated by OpenAI API]")) {
            setReportSource("OpenAI API")
            // Remove the marker from the displayed report
            setReport(result.report.replace("[Generated by OpenAI API]", "").trim())
          } else if (result.report.includes("[Generated by Mock Generator")) {
            setReportSource("Mock Generator (OpenAI API call failed)")
            // Remove the marker from the displayed report
            setReport(result.report.replace(/\[Generated by Mock Generator.*\]/g, "").trim())
          } else {
            setReport(result.report)
            setReportSource("Unknown")
          }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (reportResult.error) {
          ...
          setDebugInfo(reportResult.details)
        } else if (reportResult.report) {
          // Check if the report has a source marker
          if (reportResult.report.includes("[Generated by OpenAI API]")) {
            setReportSource("OpenAI API")
            // Remove the marker from the displayed report
            setReport(reportResult.report.replace("[Generated by OpenAI API]", "").trim())
          } else if (reportResult.report.includes("[Generated by Mock Generator")) {
            setReportSource("Mock Generator (OpenAI API call failed)")
            // Remove the marker from the displayed report
            setReport(reportResult.report.replace(/\[Generated by Mock Generator.*\]/g, "").trim())
          } else {
              ...
              {reportSource && (
                <div
                  className={`mb-4 p-3 rounded-md flex items-start ${
                    reportSource === "OpenAI API" ? "bg-green-50 text-green-800" : "bg-yellow-50 text-yellow-800"
                  }`}
                >
                  <Info className="h-5 w-5 mr-2 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-medium">Report Source: {reportSource}</p>
                    {reportSource !== "OpenAI API" && (
                      <p className="text-sm mt-1">
                        The OpenAI API call failed, so a mock report was generated instead. This is why you don't see
                        the API call in your OpenAI dashboard.
                      </p>
                    )}
                  </div>
                </div>
